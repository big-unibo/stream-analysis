...
object FileWriter {
   ...
   */
  def writeFile( filename: String, writeFun : java.io.File => Unit, synchronized: Boolean = false): Unit = {
   ...
   */
  def appendCsv ( file: java.io.File, writeFun : CSVWriter => Unit): Unit = writeCsv(file, writeFun, append = true)
   ...
   * @param append true if want to append to existing file, default false
   * @param synchronized true if want thread-safe write
   */
  def printFile (file: String, writeFun : PrintWriter => Unit, append : Boolean = false, synchronized: Boolean = false) : Unit = {
    writeFile(file, f => {
