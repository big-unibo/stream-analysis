...
object NaiveConfiguration {
  def apply(pattern: QueryPattern, alpha: Double, percentageOfRecordsInQueryResults: Double = 0.05): NaiveConfiguration = streamanalysis.input.NaiveConfiguration(
    pattern = pattern,
    alpha = alpha,
